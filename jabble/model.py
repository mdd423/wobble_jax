...
class ContainerModel(Model):
    ...
    def split_p(self, p):
        p_list = [
            p[
                jnp.arange(
                    jnp.sum(self.parameters_per_model[:k]),
                    jnp.sum(self.parameters_per_model[: k + 1]),
                    dtype=int,
                )
            ]
    ...
    def save_history(self, p):
        for i, model in enumerate(self.models):
            model.save_history(
                p[
                    np.arange(
                        np.sum(self.parameters_per_model[:i]).astype(int),
                        np.sum(self.parameters_per_model[: i + 1]).astype(int),
                        dtype=int,
                    )
                ]
            )
...
def _irwin_recursion(j, k, n):
    ...
    return _irwin_recursion(j, k - 1, n) + (
        (-1) ** (n + k - j - 1)
        * math.comb(n, k)
        * math.comb(n - 1, j)
        * k ** (n - j - 1)
